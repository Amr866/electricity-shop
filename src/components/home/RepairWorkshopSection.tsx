            ...
            >
              <div>
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="text-[10px] bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-200 font-bold px-2 py-0.5 rounded border border-slate-200 dark:border-slate-700 shadow-2xs">
                    {item.badge}
                  </span>
                </div>

                <h3 className="font-extrabold text-sm text-slate-900 dark:text-white">{item.title}</h3>
                <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed mt-1.5 font-medium">{item.desc}</p>
              </div>

              <div className="pt-3 border-t border-slate-200/80 dark:border-slate-750 flex items-center justify-between text-[11px]">
                <span className="text-emerald-600 dark:text-emerald-400 font-bold flex items-center gap-1">
                  <ShieldCheck className="w-3.5 h-3.5" />
                  <span>تست کارکرد</span>
                </span>
                <span className="text-slate-600 dark:text-slate-300 font-bold text-[10px] bg-white dark:bg-slate-800 px-2 py-0.5 rounded border border-slate-200 dark:border-slate-700">
                  کارگاه نجف‌آباد
                </span>
              </div>
            </div>
