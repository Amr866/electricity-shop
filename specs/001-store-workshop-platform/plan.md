...
## Plan Extension: Storefront Technical & Local SEO Optimization (Phase 18 - 2026-09-18)

### Technical Architecture & Decisions

1. **Dedicated SEO Category Landing Pages (`FR-058`)**:
   - Implement server-rendered page at `src/app/categories/[slug]/page.tsx`:
     - Resolves category by slug (`prisma.category.findUnique({ where: { slug } })`). If not found, calls `notFound()`.
     - Fetches active products for the category (`where: { isArchived: false, categoryId: category.id }`).
     - Dynamic `generateMetadata`: Exports localized title `${category.name} | فروشگاه برق و صنعت شیاسی نجف‌آباد`, meta description, OpenGraph tags, and canonical URL (`${baseUrl}/categories/${category.slug}`).
     - Injects `BreadcrumbList` JSON-LD schema linking Home -> Categories -> Category Name.
     - Renders responsive H1 title, category description, breadcrumbs, and `ProductCard` grid, eliminating 404 crawl errors generated by `src/app/sitemap.ts`.

2. **Dedicated Server Layouts for Client Pages (`FR-059`)**:
   - Add pure server-component `layout.tsx` files inside client route directories:
     - `src/app/repair-service/layout.tsx`: Title *"خدمات تخصصی تعمیرات و سیم‌پیچی الکتروموتور | کارگاه برق شیاسی"*, canonical `/repair-service`.
     - `src/app/contact/layout.tsx`: Title *"تماس با ما و آدرس کارگاه نجف‌آباد | فروشگاه برق شیاسی"*, canonical `/contact`.
     - `src/app/faq/layout.tsx`: Title *"سوالات متداول خرید تجهیزات و تعمیرات برق | کارگاه شیاسی"*, canonical `/faq`.
     - `src/app/price-lists/layout.tsx`: Title *"لیست قیمت سیم و کابل و ملزومات برق صنعتی | فروشگاه شیاسی"*, canonical `/price-lists`.
     - `src/app/bom-upload/layout.tsx`: Title *"استعلام قیمت و ارسال پیش‌فاکتور (BOM) | فروشگاه برق شیاسی"*, canonical `/bom-upload`.
   - Each layout exports strongly-typed `Metadata` with Persian titles, descriptions, canonical URLs, and OpenGraph definitions while cleanly passing `children` to existing client views.

3. **Schema.org Structured Data Suite & Domain Unification (`FR-060`)**:
   - Implement `JsonLd` component (`src/components/seo/JsonLd.tsx`) and schema generators:
     - `ElectronicsStore` / `LocalBusiness` schema with Najafabad verified NAP (National Address & Phone):
       - Name: "فروشگاه و کارگاه تخصصی برق شیاسی"
       - Telephone: `03142626116` / `+983142626116`
       - Address: Najafabad, Isfahan Province, Iran
       - GeoCoordinates: latitude `32.6365457`, longitude `51.3551911`
       - Opening Hours: Mo-Sa 08:30-20:30
       - PriceRange: `IRR`
     - `FAQPage` schema on `/faq` and `/repair-service` declaring question/answer pairs for Google rich snippet carousels.
     - `BreadcrumbList` schema on product detail and category landing pages.
     - Unify domain resolution: replace all hardcoded `shiasi-store.ir` references with dynamic `process.env.NEXT_PUBLIC_APP_URL || "https://shiasi-electric.ir"`.

4. **Dynamic Catalog Metadata & Core Web Vitals LCP Priority (`FR-061`)**:
   - In `src/app/products/page.tsx`:
     - Implement `generateMetadata({ searchParams })` generating contextual SEO titles and descriptions based on active filters (category, brand, or search query `q`), while fixing canonical to `${baseUrl}/products` to prevent duplicate indexing.
   - In `src/components/product/ProductDetailView.tsx`:
     - Add `priority` attribute to the primary product gallery image to eliminate image fetch delay and boost Largest Contentful Paint (LCP).
   - In `src/components/home/HeroBanner.tsx`:
     - Ensure hero banner images declare `priority` for above-the-fold optimization.
