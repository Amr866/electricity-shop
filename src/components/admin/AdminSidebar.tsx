...
export function AdminSidebar({
  ...
  const links = allLinks.filter((link) => {
    if (link.rootOnly) {
      return Boolean(isRootOwner);
    }
    if (isRootOwner || permissions.includes("ALL")) {
      return true;
    }
    if (link.module) {
      return permissions.includes(link.module);
    }
    return true;
  });
    ...
    <>
      ...
      <div className="md:hidden bg-slate-900 border-b border-slate-800 p-3.5 flex items-center justify-between sticky top-0 z-40">
        <div className="flex items-center gap-2.5">
          <button
            ...
            className="min-h-[44px] min-w-[44px] p-2 bg-slate-800 rounded-xl text-slate-200 hover:text-white flex items-center justify-center focus:ring-2 focus:ring-primary-500 focus:outline-none"
        ...
        <Link
          href="/"
          className="min-h-[44px] bg-slate-800 hover:bg-slate-700 text-amber-400 text-[11px] font-bold px-3 py-1.5 rounded-xl flex items-center gap-1 focus:ring-2 focus:ring-primary-500 focus:outline-none"
        >
        ...
        {links.map((link) => {
          ...
          return (
            <Link
              key={link.href}
              href={link.href}
              className={`min-h-[44px] flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[11px] font-bold whitespace-nowrap transition-all shrink-0 focus:ring-2 focus:ring-primary-500 focus:outline-none ${
                isActive
            ...
            {links.map((link) => {
              ...
              return (
                <Link
                  ...
                  className={`min-h-[44px] flex items-center gap-3 p-3 rounded-2xl transition-all focus:ring-2 focus:ring-primary-500 focus:outline-none ${
                    isActive
      ...
      <aside className="hidden md:flex w-64 bg-slate-900 border-l border-slate-800 p-5 space-y-6 shrink-0 flex-col justify-between min-h-screen">
        <div className="space-y-6">
          {/* Logo & Store Info */}
          <Link href="/" className="flex items-center gap-2.5 pb-4 border-b border-slate-800 group focus:ring-2 focus:ring-primary-500 focus:outline-none rounded-xl">
            ...
            {links.map((link) => {
              ...
              return (
                <Link
                  key={link.href}
                  href={link.href}
                  className={`min-h-[44px] flex items-center gap-2.5 px-3.5 py-2.5 rounded-xl transition-all focus:ring-2 focus:ring-primary-500 focus:outline-none ${
                    isActive
        ...
        <div className="pt-4 border-t border-slate-800">
          <Link
            href="/"
            className="min-h-[44px] flex items-center justify-between text-xs text-slate-400 hover:text-amber-400 p-2.5 rounded-xl hover:bg-slate-800/50 transition-colors focus:ring-2 focus:ring-primary-500 focus:outline-none"
          >
