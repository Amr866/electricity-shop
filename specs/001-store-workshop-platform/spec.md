...
- Q: Where should the admin review, approve, or deny customer comments and ratings? → A: Hybrid Reviews Moderation Architecture: A dedicated top-level admin portal at /admin/reviews accessible via the admin sidebar with pending/approved filter tabs, 1-click approve/deny actions, and bulk delete, combined with quick review access and moderation shortcuts directly from the products catalog.
- Q: How should optional product pricing units be managed and presented? → A: Configurable Pricing Unit with Presets: The admin product editor provides quick preset selection chips (عدد، متر، کلاف، شاخه، کیلوگرم، بسته) along with custom text input; if omitted, products display clean currency without unit suffix; when configured (e.g. priceUnit: "متر"), the unit displays as a suffix (/متر) in catalog cards, product detail price headers, and sticky buy bars.
- Q: How should order deletion options be provided to the store administrator in the orders management console? → A: Dual Purge Controls with High-Friction Confirmation: The admin order table provides both "حذف سفارش‌های تستی / نمونه" (deleting only sample/test demo orders) and "حذف تمامی سفارش‌های سیستم (پاکسازی کلی)" (deleting all orders across the database); both actions trigger ConfirmDeleteModal with Persian "حذف" typing friction and cascade-deletion of associated order items.

### Session 2026-09-18
- Q: How should existing module import paths across the codebase be handled during the restructuring of src/lib/? → A: Backward-Compatible Facades: Place modules into focused submodules (src/lib/core/, domain/, admin/, utils/) while maintaining lightweight re-export wrappers at the src/lib/ root so zero existing imports break and no shotgun surgery occurs.
- Q: How should unit, integration, and end-to-end tests be organized across the repository? → A: Consolidated Three-Tier Test Hierarchy: All test suites are organized under tests/ (tests/unit/, tests/integration/, tests/e2e/), src/lib/__tests__/ is pruned, package.json scripts (test, test:unit, test:integration) are updated with precise target paths, and all test imports are updated to guarantee seamless execution.
- Q: Should all redundant compiled .js twin files in src/lib/ be deleted in favor of direct TypeScript imports in test suites? → A: Purge .js Twins: Delete all compiled .js files from src/lib/ and configure all test suites to import .ts modules directly via tsx, eliminating duplicate file maintenance and preventing desynchronization bugs.
- Q: How should the operational and migration scripts in scripts/ be structured? → A: Categorized Subdirectories: Organize scripts into scripts/migrations/ (data/schema transitions), scripts/seeds/ (administrative and user seeders), and scripts/tools/ (diagnostic audits, BOM generators), clearly separating historical migrations from reusable operational utilities.
- Q: Which caching architecture tier should the platform implement to maximize page speed and query response times? → A: Native Next.js 15 In-App Caching & ISR: Utilize Next.js 15 Data Cache (unstable_cache) with tag-based on-demand revalidation (revalidateTag) for category metadata and distinct brands, paired with Incremental Static Regeneration (ISR revalidate = 300) and generateStaticParams for product detail pages, completely eliminating the operational and hosting overhead of an external Redis service.
- Q: Which database indexing strategy should be added to the PostgreSQL Product model to accelerate catalog filtering and sorting? → A: Targeted High-Frequency Composite Indexes: Add composite indexes on [isArchived, createdAt], [isArchived, price], and [isArchived, categoryId] in the Prisma schema to eliminate full table scans and in-memory sorts across 95% of customer storefront queries.
- Q: Which search matching field scope should the multi-keyword search engine query to maximize search performance and result relevance? → A: High-Relevance Commercial Scope: Target search tokens across name, sku, mpn, brand, and shortDesc, omitting lengthy raw HTML description content to eliminate heavy text scans, reduce database CPU load, and return faster, more accurate commercial matches.
- Q: Which Persian font loading configuration should be implemented for Vazirmatn to optimize mobile First Contentful Paint (FCP) across Iranian mobile networks? → A: Streamlined 4-Weight Typography: Configure next/font/google Vazirmatn with exactly 4 core weights (400, 500, 700, 900), display: "swap", and preload: true, reducing font network transfer by ~35% while maintaining complete typographic fidelity across RTL mobile interfaces.
- Q: How should the platform invalidate in-app cache and ISR pages when administrators update product prices, stock, or categories? → A: Automated Tag & Path Revalidation: Automatically trigger revalidateTag('catalog-metadata') and revalidatePath('/products/[slug]') inside admin mutation API route handlers whenever a product or category is created, edited, or archived, ensuring instantaneous store-wide consistency.
- Q: How should the category URL architecture and sitemap be structured to eliminate 404 crawl errors and maximize category keyword rankings? → A: Dedicated Clean Category Routes (/categories/[slug]): Implement dedicated server-rendered category landing pages at src/app/categories/[slug]/page.tsx featuring category-specific H1 titles, meta descriptions, BreadcrumbList schema, and pre-filtered product catalogs, perfectly harmonizing with src/app/sitemap.ts and eliminating 404 crawl errors.
- Q: How should metadata, canonical links, and OpenGraph tags be implemented for interactive Client Component pages (/repair-service, /contact, /faq, /price-lists, /bom-upload)? → A: Dedicated Server layout.tsx Files: Add lightweight layout.tsx files (pure server components) within each route folder exporting complete Metadata, canonical URLs, and OpenGraph definitions without requiring additional servers, external infrastructure, or refactoring existing client page code.
- Q: Should the National Building Code Chapter 13 Cable Calculator be provided with a dedicated standalone tool page (/tools/cable-calculator) alongside the homepage widget? → A: Retain Homepage Anchor Only (/#calculator): Keep the engineering cable calculator exclusively integrated on the homepage section with deep-link anchor support (/#calculator), eliminating redundant route maintenance and keeping all core commercial and engineering tools unified on the primary storefront.
- Q: Which Schema.org structured data types should be implemented to capture local search visibility and Google rich snippets? → A: Complete LocalBusiness, FAQPage & Breadcrumb Suite: Standardize and inject ElectronicsStore (with Najafabad coordinates, phone, physical address, and operating hours), FAQPage schema on /faq and /repair-service, and BreadcrumbList on product and category views, centrally resolving all canonical URLs and OpenGraph domains via process.env.NEXT_PUBLIC_APP_URL.
- Q: How should dynamic metadata and canonical URLs be structured for the main product catalog (/products) during search and filtering, and how should image Largest Contentful Paint (LCP) be optimized? → A: Dynamic Contextual Metadata & LCP Priority: Implement generateMetadata in src/app/products/page.tsx producing tailored Persian titles and meta descriptions for category, brand, and search filters with clean canonical self-referencing URLs, and apply the priority property to above-the-fold hero banners and primary product images in ProductDetailView to optimize Google Core Web Vitals LCP metrics.

### Session 2026-09-19
- Q: Where and how should administrators be allowed to change their passwords and provision new admin accounts? → A: In-Portal Self-Service & Multi-Admin Management with Re-Auth Guard: Logged-in admins can change their own password at /admin/settings (requires current password verification, minimum 8 characters with complexity rules, sliding-window rate limiting, and revoking stale sessions). Admins can also add or promote new administrators from a dedicated /admin/users interface, guarded by mandatory re-authentication of the executing admin's current password. The server-side CLI script (scripts/seeds/create-admin.js) is preserved as an offline root administrative tool.
- Q: When adding multiple administrators, how should account authority and deletion/demotion protections be governed between the store owner and secondary admins? → A: Protected Root Owner with Dynamic Scoped Permissions for Secondary Admins: The primary store owner account (associated with ADMIN_PHONES, e.g. 09136260072) holds immutable SuperAdmin authority and cannot be edited, demoted, or deleted by any other administrator. Only the Root Owner/SuperAdmin can create, suspend, or revoke secondary admin accounts, and can dynamically configure and modify the operational permissions assigned to each secondary admin (e.g., full access vs scoped access to Store Catalog, Order Fulfillment, Workshop Repairs, or Customer Reviews). Secondary admins can only manage their permitted operational modules and rotate their own password.
- Q: When an administrator changes their password, how should active login sessions across other devices and browsers be handled? → A: Global Invalidation of Secondary Devices via tokenVersion: Incrementing a tokenVersion field on the User record upon password rotation immediately invalidates all active sessions on other devices and browsers, forcibly terminating stale or compromised tokens upon their next authenticated API call, while seamlessly refreshing the active browser session with the incremented tokenVersion.
- Q: When the Root Owner provisions a new administrator account, how should the new admin's initial credentials and account activation be established? → A: Owner-Defined Initial Password with Prompt for Rotation: The Root Owner supplies the new admin's full name, mobile number, assigned domain modules, and sets a strong initial password (at least 8 characters with letters and numbers) in /admin/users, guarded by Root re-authentication. The new admin logs in using their phone and initial password, and is guided to change it to a private password via /admin/settings.
- Q: What authentication factor requirements should govern administrative access when signing into the /admin portal? → A: Strict Password-Gated Admin Access: Administrative sessions strictly require Phone + Password (protected by sliding-window rate limiting). SMS OTP alone cannot grant administrative privileges or elevate a user session to ADMIN, completely eliminating SIM swap and SMS interception vulnerabilities. SMS OTP is reserved exclusively for customer accounts and verified admin password recovery.
- Q: How should UI/UX Pro Max accessibility, touch interaction, and Persian RTL layout standards be enforced across administrative authentication and settings interfaces (/admin/settings, /admin/users)? → A: Accessible RTL Controls & 44px Tap Targets: Form inputs in /admin/settings must provide visible labels, proper autocomplete attributes (current-password, new-password), preserved focus rings (focus:ring-2 focus:ring-primary-500), live complexity meters, and 4.5:1 contrast error messaging (role="alert"). The password reveal toggle must be positioned at RTL inline-start (left-3) with a minimum 44×44px tap target and descriptive aria-label. Phone numbers in the /admin/users table must be isolated via <bdi dir="ltr" className="font-mono"> to prevent directional numeral corruption.
- Q: When an administrator account is suspended by the Root Owner, how should session revocation and access restriction behave? → A: Immediate Global Suspension Invalidation: Suspending a secondary admin account (isSuspended: true) atomically increments their tokenVersion, immediately revoking all active JWT sessions on secondary devices upon their next API request (returning HTTP 401 Unauthorized), while blocking any subsequent password authentication attempts with an explicit Persian suspension alert.
- Q: How should Test-Driven Development (TDD) public seams and verification contracts be established for the administrative governance subsystem? → A: Contract-First Public Seams: Tests must verify behavior exclusively at public API and authentication seams (POST /api/admin/change-password, /api/admin/users, NextAuth authorize() credentials provider) without mock-coupling internal database queries or scrypt algorithms. Expected error codes (401, 403, 429) must be asserted against contract literals, following red-green-refactor vertical slices.
...
2. **Given** a user enters `"موتور موتوژن ۳/۴"` in the search bar, **When** characters are typed, **Then** the system normalizes Persian and Arabic letter variants (`ي` to `ی`, `ك` to `ک`), matches across high-relevance attributes (title, brand, SKU, MPN, short summary), and returns relevant results within 100 milliseconds without scanning heavy HTML description bodies.
...
- **Admin Password Brute-Force & Credential Interception**: Repeated failed password verification attempts on `/api/admin/change-password` or admin management endpoints are throttled after 5 consecutive failures within a 15-minute sliding window per IP and account; API responses return generic error messaging without revealing account existence or hash details, preventing credential harvesting and enumeration attacks.
...
- **Suspended Admin Active Token Replay**: If a suspended administrator attempts to make an API request with a previously valid JWT token, the server detects that the user's `isSuspended` flag is true and that `tokenVersion` has incremented upon suspension, rejects the request with HTTP 401 Unauthorized, terminates the session, and redirects to the login screen with a Persian notification: *"حساب کاربری شما توسط مدیر ارشد به حالت تعلیق درآمده است"*.
- **Admin Password Eye Toggle Misalignment in RTL**: On mobile and desktop browsers in Persian RTL mode, password toggle eye buttons positioned via generic `right-*` classes overlap with text or get pushed offscreen; the system explicitly bounds eye toggle icons to inline-start (`left-3` in RTL) with a minimum 44×44px tap target to guarantee zero touch overlap.
...
- **FR-046**: The system MUST provide an administrative customer reviews moderation hub (`/admin/reviews`) and inline product review moderation shortcuts, enabling administrators to inspect submissions, approve (`isVerified: true`) or deny/unpublish (`isVerified: false`) reviews, permanently purge abusive feedback with confirmation, and automatically recompute the associated product's average rating and verified review count upon every moderation action.
- **FR-047**: The product catalog and administrative editor MUST support an optional pricing unit attribute (`priceUnit`). The product editor MUST offer fast preset chips (عدد، متر، کلاف، شاخه، کیلوگرم، بسته) and custom text entry. When `priceUnit` is populated, storefront catalog cards, product detail price displays, and sticky mobile action bars MUST append the unit suffix (e.g. `/متر`). When omitted or null, the system MUST render the clean currency value without any unit suffix.
- **FR-048**: The administrative orders management console MUST provide dual deletion actions: (1) Sample Order Purge targeting simulated/demo orders, and (2) Total Order Purge clearing all orders across the database. Both actions MUST execute cascade deletion of associated order items and enforce Smart Friction confirmation dialogs requiring Persian "حذف" text entry before executing the destructive purge.
- **FR-049**: The codebase architecture MUST organize utility, infrastructure, domain, and administrative modules into deep sub-packages (`src/lib/core/`, `src/lib/domain/`, `src/lib/admin/`, `src/lib/utils/`) following deep-module design principles. The system MUST maintain zero-breakage facade re-exports at the `src/lib/` root to preserve backward compatibility for all existing `@/lib/...` call sites without requiring broad codebase edits.
- **FR-050**: The testing infrastructure MUST organize all automated tests into a unified, three-tiered structure under `tests/` (`tests/unit/`, `tests/integration/`, `tests/e2e/`), removing embedded test folders from `src/lib/`. The `package.json` test triggers (`test`, `test:unit`, `test:integration`) and test runner invocations MUST point to the new paths, with all test import paths updated to ensure 100% passing execution.
- **FR-051**: The codebase MUST eliminate all duplicate compiled `.js` files from `src/lib/` (`admin-order-purge.js`, `admin-review-moderation.js`, `admin-bom-guard.js`, `admin-product-guard.js`, `admin-repair-guard.js`). All test suites and scripts MUST import `.ts` source modules directly using `tsx` runtime execution.
- **FR-052**: The operational scripts infrastructure MUST organize standalone scripts into categorized subdirectories (`scripts/migrations/`, `scripts/seeds/`, `scripts/tools/`), separating one-off historical data transitions from ongoing operational and simulation utilities, while updating any `package.json` script triggers to their new relative paths.
- **FR-053**: The platform MUST implement in-app Data Cache and Incremental Static Regeneration (ISR) without external Redis infrastructure dependencies. Category metadata, product counts, and distinct brand listings MUST be cached using Next.js `unstable_cache` with tag-based on-demand revalidation (`catalog-metadata`) triggered upon product or category administrative modifications. Product detail pages (`/products/[slug]`) MUST implement ISR (`revalidate = 300`) and pre-render active catalog slugs via `generateStaticParams` to deliver sub-50ms static page responses directly from memory.
- **FR-054**: The database schema MUST define composite indexes on the `Product` entity for the most frequent query patterns: `[isArchived, createdAt]` for default recency sorting, `[isArchived, price]` for price ordering, and `[isArchived, categoryId]` for category filtering, preventing full table scans and keeping catalog query execution times under 20ms.
- **FR-055**: The storefront multi-keyword search engine MUST query normalized Persian search tokens against high-relevance catalog attributes: product title (`name`), stock keeping unit (`sku`), manufacturer part number (`mpn`), brand name (`brand`), and summary teaser (`shortDesc`). The search query MUST NOT perform wildcard text scans on extensive raw HTML product descriptions, ensuring multi-token search queries execute in under 100 milliseconds.
- **FR-056**: The root application layout (`src/app/layout.tsx`) MUST configure the Vazirmatn Google font using exactly four core typographic weights: Regular (`400`) for body copy, Medium (`500`) for controls and meta-labels, Bold (`700`) for component titles and badges, and Black (`900`) for high-emphasis currency headers and numerals, with `display: "swap"` and `preload: true`, eliminating redundant font downloads and minimizing mobile First Contentful Paint (FCP) latency.
- **FR-057**: Administrative product and category mutation API route handlers (`/api/admin/products` and `/api/admin/categories`) MUST execute automated on-demand cache invalidation using `revalidateTag('catalog-metadata')` and `revalidatePath('/products/[slug]')` upon every successful create, update, or archive operation, ensuring changes to prices, inventory, or categorization reflect immediately across the public storefront.
- **FR-058**: The application MUST implement dedicated SEO category landing pages at `/categories/[slug]` (`src/app/categories/[slug]/page.tsx`) that pre-filter products by category, render category-specific meta titles and descriptions via `generateMetadata`, display BreadcrumbList structured data, and match the URL structure declared in `src/app/sitemap.ts`, eliminating 404 crawl errors.
- **FR-059**: All public interactive routes declaring client-side interactivity (`/repair-service`, `/contact`, `/faq`, `/price-lists`, `/bom-upload`) MUST include a dedicated server-side `layout.tsx` exporting rich `Metadata` (localized Persian title, targeted meta description, canonical URL, and OpenGraph object), ensuring search engines index comprehensive metadata without requiring extra server infrastructure.
- **FR-060**: The platform MUST inject standardized Schema.org JSON-LD structured data across key views: `ElectronicsStore` on the homepage/footer containing the verified Najafabad physical address, telephone (`03142626116`), GPS coordinates (`32.6365457, 51.3551911`), and operating hours; `FAQPage` schema on `/faq` and `/repair-service` for accordion search snippets; and `BreadcrumbList` on product and category views. All structured data and canonical links MUST dynamically resolve against `process.env.NEXT_PUBLIC_APP_URL`, eliminating hardcoded domain mismatches.
- **FR-061**: The storefront catalog (`src/app/products/page.tsx`) MUST implement dynamic `generateMetadata` constructing contextual Persian titles and meta descriptions based on active filter parameters (category, brand, or search query), enforcing a clean canonical URL (`alternates.canonical: '/products'`) to prevent duplicate content penalties across query permutations. Above-the-fold hero banners and primary product images in `ProductDetailView` MUST declare the `priority` property to reduce Largest Contentful Paint (LCP) latency.
- **FR-062**: The administrative portal MUST provide a secure self-service password rotation interface at `/admin/settings` (`/api/admin/change-password`). Changing an admin password MUST strictly require verification of the existing current password using timing-safe comparison, enforce a minimum length of 8 characters with alphanumeric complexity (`/^(?=.*[A-Za-z])(?=.*\d).{8,}$/`), apply sliding-window rate limiting (maximum 5 attempts per 15 minutes), and update the password as a cryptographically salted scrypt hash (`src/lib/core/password.ts`). The interface MUST adhere to UI/UX Pro Max standards: visible `<label>` elements, proper `autocomplete` attributes (`current-password`, `new-password`), preserved high-contrast focus rings (`focus:ring-2 focus:ring-primary-500`), live complexity validation chips, 4.5:1 contrast error feedback with `role="alert"`, and an eye toggle button positioned at RTL inline-start (`left-3`) with a minimum 44×44px tap target and descriptive `aria-label`. The API and application loggers MUST NEVER return, log, or serialize plaintext passwords or password hashes in responses or stack traces.
- **FR-063**: The administrative portal MUST provide an administrator governance interface at `/admin/users` (`/api/admin/users`) accessible exclusively to the Root Owner / SuperAdmin. The Root Owner account (associated with `ADMIN_PHONES`, e.g. `09136260072`) MUST be strictly immutable against deletion, suspension, or demotion. Only the Root Owner can create, suspend, or revoke secondary administrator accounts. Executing any admin provisioning, permission update, or account suspension action MUST require mandatory re-authentication of the Root Owner's current password via an accessible confirmation modal implementing WCAG focus traps, Escape dismissal, and 44×44px action buttons to prevent session-hijack escalation.
- **FR-064**: The Root Owner MUST be able to dynamically configure and update scoped operational permissions for each secondary administrator across four granular domain modules: `CATALOG` (products, categories, pricing units), `ORDERS` (order fulfillment, tracking numbers, invoices), `REPAIRS` (technical workshop intake, diagnoses, cost estimates), and `REVIEWS` (customer rating and feedback moderation). Secondary administrators MUST be restricted to their assigned operational domains and their self-service password rotation view, preventing unauthorized lateral privilege escalation. Data tables displaying administrative user details MUST wrap phone numbers in `<bdi dir="ltr" className="font-mono">` to prevent directional RTL text corruption.
- **FR-065**: The authentication and session management subsystem MUST implement global session invalidation using an integer `tokenVersion` column on the `User` model. Upon every successful password update, administrative password reset, or account suspension (`isSuspended: true`), `tokenVersion` MUST be atomically incremented in the database. Active JWT tokens presented by secondary devices, concurrent browsers, or unauthorized clients carrying an outdated `tokenVersion` MUST be rejected immediately (HTTP 401 Unauthorized), terminating all stale sessions worldwide without requiring an external Redis store.
- **FR-066**: When provisioning a new administrator account at `/admin/users` (`/api/admin/users`), the Root Owner MUST provide the full name, normalized Iranian mobile number, authorized domain permissions, and an initial strong password (minimum 8 characters containing both letters and numbers). The platform MUST immediately salt and hash the initial password via `src/lib/core/password.ts`, store it without logging or exposing the plaintext, set `isVerified: true` and `role: "ADMIN"`, and guide the new administrator toward personal credential rotation on their first login.
- **FR-067**: The authentication provider (`src/lib/core/auth.ts`) MUST strictly enforce password-gated authentication for all administrative logins. SMS OTP verification alone MUST NOT grant administrative session privileges (`role: "ADMIN"`), even if the phone number matches an administrator or `ADMIN_PHONES`. Accessing the `/admin` console MUST require successful authentication via Phone and Password, protected by sliding-window rate limiting (maximum 5 failed attempts per 15 minutes), preventing SIM swap and SMS interception attacks from compromising the back office. The test suite MUST verify these authorization guarantees at public API and NextAuth provider seams without database mock-coupling, following Test-Driven Development (TDD) principles.

---

### Key Entities

- **Product**: Unique identifier, title, URL slug, SKU, manufacturer part number (MPN), category reference, retail price, promotional price, discount percentage, optional pricing unit (`priceUnit`), available stock quantity, minimum order quantity (MOQ), brand name, warranty terms, country of origin, delivery badges, `isArchived` retention flag, and creation/update timestamps.
- **Review**: Unique identifier, product reference, author name, rating (1-5), comment body, verified moderation flag (`isVerified`), and submission timestamp.
...
- **User & VerificationToken**: Full name, normalized mobile number, email address, password hash (salted scrypt, strictly omitted from public/client queries), user role (`CUSTOMER`, `ADMIN`), token version (`tokenVersion` integer incremented upon password rotation for global session revocation), admin permissions scope (JSON string array of authorized modules: `CATALOG`, `ORDERS`, `REPAIRS`, `REVIEWS`, or `ALL` for Root Owner), postal address, city, postal code, national identity number, company name, economic code, token code, expiration timestamp, resend attempt count, cooldown until timestamp, and session state.
...
- **SC-013**: 100% of public storefront pages achieve sub-100ms server TTFB from cache and First Contentful Paint (FCP) under 1.2 seconds on standard 4G mobile networks.
